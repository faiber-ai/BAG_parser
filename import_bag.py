...
def main():
    ...
    utils.print_log(
        f"BAG parser version {config.version} | {config.version_date} | {config.cpu_cores_used} CPU cores"
    )
    utils.print_log(
        f"start: parse BAG XML '{config.file_bag}' to sqlite database '{config.file_db_sqlite}'"
    )
    ...
    with DatabaseSqlite() as db_sqlite:
        ...
        if config.create_adressen_table:
            if not config.active_only:
                utils.print_log(
                    "addresses table is only created if active_only=True in config",
                    True,
                )
            else:
