...
def parse_xml_file(
    file_xml, tag_name, data_init, object_tag_name, db_fields, db_tag_parent_fields
):
    ...
    match tag_name:
        case "Woonplaats":
            ...
            has_geometry = True
            save_function = lambda db, row: db.save_woonplaats(row)
        case "GemeenteWoonplaatsRelatie":
            save_function = lambda db, row: db.save_gemeente_woonplaats(row)
        case "OpenbareRuimte":
            status_active = "Naamgeving uitgegeven"
            save_function = lambda db, row: db.save_openbare_ruimte(row)
        case "Nummeraanduiding":
            status_active = "Naamgeving uitgegeven"
            save_function = lambda db, row: db.save_nummer(row)
        case "Pand":
            save_function = lambda db, row: db.save_pand(row)
        case "Verblijfsobject":
            coordinates_field = "pos"
            has_geometry = True
            save_function = lambda db, row: db.save_verblijfsobject(row)
        case "Ligplaats":
            coordinates_field = "geometry"
            has_geometry = True
            save_function = lambda db, row: db.save_ligplaats(row)
        case "Standplaats":
            coordinates_field = "geometry"
            has_geometry = True
            save_function = lambda db, row: db.save_standplaats(row)
        case _:
    ...
    # Save data to SQLite using context manager
    with DatabaseSqlite() as db_sqlite:
        # Save data to SQLite
        for db_row in db_data:
            save_function(db_sqlite, db_row)
        db_sqlite.commit()
...
def add_coordinates(rows, field_name):
    for i, row in enumerate(rows):
        if row[field_name]:
            ...
            [row["latitude"], row["longitude"]] = rijksdriehoek.rijksdriehoek_to_wgs84(
                row["rd_x"], row["rd_y"]
            )
...
class BagParser:
    ...
    def parse(self, tag_name):
        ...
        if self.tag_name == "Woonplaats":
            ...
            self.db_tag_parent_fields = {
                ...
                + ns_gwr_product
                + "identificatie": "woonplaats_id",
                ns_gwr_product
                + "gerelateerdeGemeente"
                + ns_gwr_product
                + "identificatie": "gemeente_id",
            }
            ...
            self.db_tag_parent_fields = {
                ...
                + "heeftAlsHoofdadres"
                + ns_objecten_ref
                + "NummeraanduidingRef": "nummer_id",
                ns_objecten
                + "heeftAlsNevenadres"
                + ns_objecten_ref
                + "NummeraanduidingRef": "nevenadressen",
            }
        ...
        utils.print_log(
            f"ready: parse XML {self.tag_name} | {time_elapsed} "
    ...
    def add_gemeenten_into_woonplaatsen(self):
        if not config.active_only:
            utils.print_log(
                "gemeente_id is only added to woonplaatsen if active_only=True in config",
                True,
            )
            return
    ...
    def __update_xml_status(self, final=False):
        if (
            final
            or (self.gui_time is None)
            or (time.perf_counter() - self.gui_time > 0.5)
        ):
            ...
            utils.print_progress_bar(
                self.count_xml_files, self.total_xml_files, bar_text, final
            )
